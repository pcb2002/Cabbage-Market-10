...
| 공개 API | 인증 필요 API |
|---|---|
| 회원가입, 로그인, 토큰 재발급, 상품 목록·상세·검색(v1/v2), 카테고리, 회원 공개 프로필 | 로그아웃, 내 정보, 상품 등록·수정·게시·삭제, 상품 이미지 업로드·수정·삭제, 좋아요, 문의 작성·수정·삭제, 팔로우, 채팅, 리뷰, 입찰 |
...
| 상품 검색 v2 | GET | `/api/v2/items/search` | 불필요 | `keyword`, `categoryId`, `tradeStatus`, `page`, `size`, `sort` 선택 | `200 OK` |
