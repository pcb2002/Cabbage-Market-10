...
- `status`는 계정 상태를 표현한다.
- `is_verified`는 인증 완료 여부다.
- 탈퇴 회원은 `is_deleted`을 기록하고 일반 조회에서 제외한다.
...
- `buyer_id`는 직거래 상품의 거래 완료 구매자 Client를 참조한다.
...
- 직거래 상품을 `SOLD_OUT`으로 변경할 때는 `buyerId`를 함께 전달해야 한다.
...
- 입찰 동시성 제어는 상품 단위 Redis 기반 Redisson 분산 락으로 처리한다.
