...
- JWT에는 `jti`를 포함하고, 로그아웃·강제 만료된 Access Token의 `jti`는 Redis 블랙리스트에 저장한다.
- 로그아웃 시 Access Token은 Redis 블랙리스트에 등록하고 Refresh Token에는 만료 쿠키를 내려준다.
...
- Refresh Token은 서버 저장소나 블랙리스트로 별도 관리하지 않으며, 로그아웃은 현재 클라이언트의 만료 쿠키 기준으로 처리한다.
- 이미 유출된 Refresh Token의 서버 측 즉시 무효화는 보장하지 않는다.
